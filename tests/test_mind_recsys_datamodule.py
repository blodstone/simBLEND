...
def collate_args():
    ...
    glove_embedding_path = glove_path + '/glove.6B.50d.txt'
    glove_embedding = {}
    ...
    return {
        ...
        "glove_embedding": embedding_matrix,
        "use_glove": True,
        "use_tokenizer": False,
    }
...
def test_train_dataset_getitem(train_dataset):
    dataset = train_dataset
    history, history_category, history_subcategory, candidates, candidates_category, candidates_subcategory, user_ids, labels = dataset[0]
    assert candidates == ["The Cost of Trump's Aid Freeze in the Trenches of Ukraine's War Lt. Ivan Molchanets peeked over a parapet of sand bags at the front line of the war in Ukraine. Next to him was an empty helmet propped up to trick snipers, already perforated with multiple holes.", "I Was An NBA Wife. Here's How It Affected My Mental Health. I felt like I was a fraud, and being an NBA wife didn't help that. In fact, it nearly destroyed me.", "Should NFL be able to fine players for criticizing officiating? Several fines came down against NFL players for criticizing officiating this week. It's a very bad look for the league."]
    assert history == ['50 Worst Habits For Belly Fat These seemingly harmless habits are holding you back and keeping you from shedding that unwanted belly fat for good.', "Dispose of unwanted prescription drugs during the DEA's Take Back Day "]
    assert history_category == [1, 1]
    assert history_subcategory == [1, 2]
    assert candidates_category == [2, 1, 3]
    assert candidates_subcategory == [3, 4, 5]
    assert user_ids == 1
    assert labels == [1, 0, 1]
...
def test_train_collate_type(train_dataset, collate_args):
 ...
 # Check dtypes (optional, but good practice)
    assert batch['user_history'].dtype == torch.float32
    assert batch['user_history_cat'].dtype == torch.int64
    assert batch['user_history_subcat'].dtype == torch.int64
    assert batch['candidates'].dtype == torch.float32
    assert batch['candidates_cat'].dtype == torch.int64
    assert batch['candidates_subcat'].dtype == torch.int64
    assert batch['user_ids'].dtype == torch.int64
    assert batch['labels'].dtype == torch.int64
    assert batch['history_mask'].dtype == torch.int64
    assert batch['candidate_mask'].dtype == torch.int64
    assert batch['padded_history_mask'].dtype == torch.int64
    assert batch['padded_candidate_mask'].dtype == torch.int64

def test_train_collate_dim(train_dataset, collate_args):
    dataset = train_dataset

    BATCH_SIZE = 6
    # Infer expected shapes from collate_args and dataset
    EMBEDDING_DIM = collate_args["glove_embedding"].shape[1]
    expected_max_history_len = 5
    expected_max_candidates_len = 4
    expected_max_token_history_len = 52
    expected_max_token_candidates_len = 52
    # Test collate
    collate_fn = DatasetCollate(**collate_args)
    dataloader = DataLoader(
            dataset,
    ...
    batch = next(dataloader.__iter__())
     ...
     # Assert shapes
    assert batch['user_history'].shape == (BATCH_SIZE, expected_max_history_len, expected_max_token_history_len, EMBEDDING_DIM), "user_history shape mismatch"
    assert batch['user_history_cat'].shape == (BATCH_SIZE, expected_max_history_len), "user_history_cat shape mismatch"
    assert batch['user_history_subcat'].shape == (BATCH_SIZE, expected_max_history_len), "user_history_subcat shape mismatch"
    
    assert batch['candidates'].shape == (BATCH_SIZE, expected_max_candidates_len,  expected_max_token_candidates_len, EMBEDDING_DIM), "candidates shape mismatch"
    
    assert batch['candidates_cat'].shape == (BATCH_SIZE, expected_max_candidates_len), "candidates_cat shape mismatch"
    assert batch['candidates_subcat'].shape == (BATCH_SIZE, expected_max_candidates_len), "candidates_subcat shape mismatch"
    assert batch['user_ids'].shape == (BATCH_SIZE,), "user_ids shape mismatch"
    assert batch['labels'].shape == (BATCH_SIZE, expected_max_candidates_len), "labels shape mismatch"
    
    assert batch['history_mask'].shape == (BATCH_SIZE, expected_max_history_len), "history_mask shape mismatch"
    assert batch['candidate_mask'].shape == (BATCH_SIZE, expected_max_candidates_len), "candidate_mask shape mismatch"
    
    assert batch['padded_history_mask'].shape == (BATCH_SIZE, expected_max_history_len, expected_max_token_history_len), "padded_history_mask shape mismatch"
    assert batch['padded_candidate_mask'].shape == (BATCH_SIZE, expected_max_candidates_len, expected_max_token_candidates_len), "padded_candidate_mask shape mismatch"
